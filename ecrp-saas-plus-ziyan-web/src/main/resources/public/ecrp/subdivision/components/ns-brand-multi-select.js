...
define(["jquery", "vue", "nui"], function ($, Vue, Nui) {
	...
	var template = `
		<div class="template-multi" nesting>
			<div class="template-multi-content" nesting>
				<el-multi :data="nodes" leaf-check-limit accordion node-key="code" :default-checked-keys="defaultCheckedKeys" :set-position="36"
					@check-change="checkChangeHandle" ref="shopNode" :cols="2" :show-Selectall="false" :props="shopProps" nesting>
				</el-multi>
			</div>
        </div>
	`;
	...
	var $template = `
		<el-dialog :title="title" v-model="dialogBrandMultilyaerVisible" size="tiny" height="400px" :vetically="true" custom-class="no-padding" :modal-append-to-body="false" :close-on-click-modal="false" nesting>
