				...
				for (var i = data.length-1; i >= 0; i--) {
					...
					if(data[i].relation != ""){
						if(data[i].condition == "notEqual"){
							...
							'<condition es_field="operate_customer.last_sms_time"  operator="smaller">' +
							'<![CDATA['+date+'T23:59:59+08:00]]>' +
							'</condition>' +
							'<condition es_field="operate_customer.last_sms_time"  operator="larger">' +
							'<![CDATA['+date+'T00:00:00+08:00]]>' +
							'</condition>' +
							 '</term>';
						}else{
							getXml +='<term es_nested="operate_customer"  relation="'+data[i].relation+'">' +
							'<condition es_field="operate_customer.last_sms_time"  operator="'+data[i].condition+'">' +
							'<![CDATA['+date+']]>' +
							'</condition>' +
							 '</term>';
						}
