//权益黑名单
var permission_black = {
	Data : function(param1,param){
			param1 = "permission_black";
			var getValue={};
			$.ajax({
				url:"getBrand",
				data:"",
			...
			var getXml = "<term>";
				for (var i = data.length-1; i >= 0; i--) {
					if(data[i].relation != ""){
						...
						'<condition es_field="customer_brand.is_right_black" column="is_right_black" operator="'+data[i].condition+'">' +
						'<![CDATA['+data[i].CDATA+']]>' +
						'</condition>' +
	...
	Sql : function (param){
			...
			for (var i = data.length-1; i >= 0; i--) {
				...
				getSqlCondition += " "+data[i].relation+" kd_customer_brand.is_right_black  =  "+data[i].CDATA+"";
