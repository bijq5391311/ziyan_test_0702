//首次互动时间
var first_get_integral_time = {
	Data : function(param1,param){
			param1 = "first_get_integral_time";
			var getValue ={};
        	getValue.relation = "";
					...
					if(data[i].relation != ""){
						if(data[i].condition == "notEqual"){
							getXml +='<term es_nested="customer_shop_rfm"  relation="'+data[i].relation+'">' +
								'<condition es_field="customer_shop_rfm.first_interact_time"  operator="larger">' +
								'<![CDATA['+dataL+']]>' +
								'</condition></term>' +
								'<term es_nested="customer_shop_rfm"  relation="or"><condition es_field="customer_shop_rfm.first_interact_time"  operator="smaller">' +
								'<![CDATA['+dataF+']]>' +
								'</condition></term>' +
								'<term es_nested="customer_shop_rfm"  relation="or"><condition es_field="customer_shop_rfm.first_interact_time"  operator="isNull">' +
								'<![CDATA[]]>' +
								'</condition>' +
								'</term>';
						}else{
							getXml +='<term es_nested="customer_shop_rfm"  relation="'+data[i].relation+'">' +
								'<condition es_field="customer_shop_rfm.first_interact_time" operator="'+data[i].condition+'">' +
								'<![CDATA['+date+']]>' +
								'</condition>' +
								'</term>';
						}
	...
	Sql : function (param){
			var name = "first_get_integral_time";
			var getSqlJoin = "";
			var getSqlCondition = "";
			var data = eval(param);
			for (var i = data.length-1; i >= 0; i--) {
