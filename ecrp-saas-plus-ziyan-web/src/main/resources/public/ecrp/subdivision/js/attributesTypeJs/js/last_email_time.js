	...
	Sql : function (param){
			...
			for (var i = data.length-1; i >= 0; i--) {
				...
				if(data[i].condition == "between"){
					getSqlCondition += " "+data[i].relation+" kd_operate_customer.last_email_time  between  '"+commonFunction.GetDateStr(parseInt(data[i].newday))+"' and  '"+commonFunction.GetDateStr(parseInt(data[i].oldday))+"' ";
				}else if(data[i].condition == "equal"){
					getSqlCondition += " "+data[i].relation+" kd_operate_customer.last_email_time = '"+data[i].date.split("T")[0]+"'";
				}else if(data[i].condition == "notEqual"){
					getSqlCondition += " "+data[i].relation+" kd_operate_customer.last_email_time != '"+data[i].date.split("T")[0]+"'";
				}else if(data[i].condition == "largerEqual"){
					getSqlCondition += " "+data[i].relation+" kd_operate_customer.last_email_time < '"+data[i].date.split("T")[0]+"'";
