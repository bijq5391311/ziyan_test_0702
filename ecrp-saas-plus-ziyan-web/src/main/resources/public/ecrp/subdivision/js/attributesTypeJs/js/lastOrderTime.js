...
var lastOrderTime = {
				...
				for (var i = data.length-1; i >= 0; i--) {
					...
					if(data[i].relation != ""){
						if(data[i].condition == "notEqual"){
							...
							'<condition es_field="customer_shop_rfm.last_order_time"  operator="larger">' +
							'<![CDATA['+date+'T23:59:59+08:00]]>' +
							'</condition></term><term relation="or" es_nested="customer_shop_rfm">' +
							'<condition es_field="customer_shop_rfm.last_order_time"  operator="smaller">' +
							'<![CDATA['+date+'T00:00:00+08:00]]>' +
								'</condition></term>' +
								'<term es_nested="customer_shop_rfm"  relation="or"><condition es_field="customer_shop_rfm.last_order_time"  operator="isNull">' +
								'<![CDATA[]]>' +
							'</condition>' +
							 '</term>';
						}else{
							getXml +='<term es_nested="customer_shop_rfm"  relation="'+data[i].relation+'">' +
							'<condition es_field="customer_shop_rfm.last_order_time"  operator="'+data[i].condition+'">' +
							'<![CDATA['+date+']]>' +
							'</condition>' +
							 '</term>';
						}
	...
	Sql : function (param){
			...
			for (var i = data.length-1; i >= 0; i--) {
				...
				getSqlCondition += " "+data[i].relation+" DATE_FORMAT(kd_customer_shop_rfm.last_order_time,'%Y-%m-%d')  "+data[i].condition+" '"+data[i].CDATA+"'";
			}
