...
define(['jquery', 'vue', 'nui'], function ($, Vue, nui) {
	...
	var $template = `
					...
					<div>
                        <bus-toggles :max-height="180" :class="isOr? 'is-double or': 'is-double'" v-if="showConditionsSelect">
                          <el-row class="tmp-choose" :gutter="20" >
                            <el-col :span="12">
                              <div class="tmp-choose__title"><el-button type="primary" @click="showChooseGoods">选择商品</el-button><span  class="total">已选择{{confirmGoodsData.length}}个商品</span></div>
                        ...
                        <!-- 选择商品弹窗结构 -->
          <el-dialog :vetically="true" title="选择商品" v-model="chooseGoodsVisible" content-min-height="475px" size="middle" :modal-append-to-body="false"  class="g-wrapper" custom-class="condition-dialog"  :close-on-click-modal="false" @close="goodsClose"  nesting>
          ...
          <el-dialog title="选择条件"  v-model="chooseConditionVisible"  size="middle" :modal-append-to-body="false"  :close-on-click-modal="false"  @close="conditionClose" nesting  class="r-wrapper" custom-class="condition-dialog">
            ...
            <el-row class="tmp-choose__condition rules"  :gutter="30" >
              <div   class="condition-left" ref="conditionItem2" >
                <el-form  ref="conditionForm" label-width="80px" :model="chooseCondition">
                  ...
                  <el-form-item label="价格区间：" v-for="(item, index) in chooseCondition.price" :key="index">
                    <el-form-grid size="md">
                      <el-row>
                        <el-col :span="11">
            ...
            <div slot="footer" class="dialog-footer">
              <el-button @click="()=>{ this.chooseConditionVisible = false}">取消</el-button>
	...
	var built_in_methods = {
		...
		ns_table_ajax: function (params) {
			...
			$.post(_this.goodsTableUrl,params).done(function (resp) {
				if (resp.success && resp.result.data.length > 0){
					_this.$set(_this,'goodsData',_this.changeItemIdToID(resp.result.data));
		...
		ns_reset: function () {
            ...
            this.searchMap.shopCodes = this.shopCodes.join(",");
			this.reload();
		},
	}
	...
	var props = {
		...
		goodsTableUrl: {
			type: String,
			default:function(){
				return ctx + "/base/common/queryKdItemList";
			}
		},
		goodsCategoryUrl:{
			type: String,
	...
	var $methods = {
		...
		showChooseGoods: function(){
			...
			this._init();
			this.$set(this,'goodsSelectedData',[].concat(this.$data.confirmGoodsData));
		},
		//打开选择条件
		...
		initDropTree: function () {
			...
			$.post(_this.goodsCategoryUrl,{shopCode:_this.shopCodes.join(",")? _this.shopCodes.join(","): null}).done(function (resp) {
	...
	// 商品选择组件定义
	Vue.component("ns-all-item-select", {
		template: $template,
		data:function (){
			return {
				goodsData: [],//商品列表数据
