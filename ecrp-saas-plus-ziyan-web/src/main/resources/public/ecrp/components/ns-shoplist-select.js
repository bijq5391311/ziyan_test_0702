...
define(['jquery', 'vue', 'nui','/public/ecrp/components/zy-ns-area.js','/public/ecrp/common/common.js','/public/ecrp/subdivision/components/ns-cascader.js'], function ($, Vue, nui,areas,common) {
            ...
            <el-row class="tmp-choose__condition"  :gutter="20">
              <el-col :span="15"  class="condition-left" ref="conditionItem">
                ...
                <el-form-grid>
               	...
               	<el-form-grid>渠道：</el-form-grid>
                <el-form-grid>
                <ns-droptree  v-model="channelValue"  
		                   			:url="'/base/common/loadChannelTree'"
		                   			placeholder="请选择" :show-checkbox="true" class="is-gray">
		        </ns-droptree>
                </el-form-grid>
                </el-form-item>
                <el-form-item>
               ...
               <el-form-item  label="门店名称：">
                ...
                <el-table ref="goodsTable" :data="shopsData" 
                 ...
                 <el-table-column  type="selection" width="40" align="center"> </el-table-column>
                  ...
                  <el-table-column v-if="shopsData==null" property="shopcode" label="" show-overflow-tooltip width="0"></el-table-column>
                </el-table>
                <el-pagination v-show="ns_pagination.enable"
                  ...
                  <el-scrollbar ref="tmpChooseScroll"
                    ...
                    <ul class="tmp-choose__goods">
                      <li v-for="(goods,index) in goodsSelectedData">
                        ...
                        <a @click="deleteSelectedGoods('goodsSelectedData',index,goods.shopcode)"><i class="g-delete bui-delete"></i></a>
                      </li>
                    </ul>
                  </el-scrollbar>
                </div>
	...
	var props = {
		...
		goodsTableUrl: {
			type: String,
			default:function(){
				return ctx + "/base/common/queryOutShopTree4New";
			}
		},
		goodsCategoryUrl:{
			type: String,
	...
	var $methods = {
		...
		deleteSelectedGoods: function($dataName,index,code){
			...
			if(this.$refs.goodsTable){
				for(var i = 0; i < this.shopsData.length; i++ ){
					if(code === this.shopsData[i].shopcode){
      ...
      args.defaultCheckedKeys.map(function (item) {
        shopCodeList.push({shopcode: item})
      })
     ...
     this.$set(this,'goodsSelectedData',[].concat(shopCodeList));
		...
		getText: function(){
			var text = []
				this.goodsSelectedData.map(function (item) {
		...
		confirmCheck: function(dropTree,type,index){
			...
			for (var i = 0; i < checkArr.length; i++) {
				...
				checkArrCode.push(checkArr[i].shopcode);
			}
		...
		uniqueArray: function (array){ 
			...
			for(var i = 0, l = array.length; i < l; i++) { 
				for(var j = i + 1; j < l; j++) 
					if (array[i].shopcode === array[j].shopcode)
						j = ++i; 
						r.push(array[i]); 
			} 
			return r; 
		...
		handleGoodsData: function(data){
            ...
            if(data.length > 0){
                for(var i =0;i<data.length;i++ ){
					shopCodes.push(data[i].shopcode);
                }
			}
			// data.map((item,index)=>{
	...
	/*	//商品列表回显
		toggleSelection: function(selected,rows){
			for(var i = 0; i<rows.length; i++){
				for(var j = 0; j<selected.length; j++){
					if(rows[i].shopcode == selected[j].shopcode ){
		...
		toggleSelection: function(selected,rows){          
			var that = this;          
			var list = [];          
			var codes = selected.length>0?selected[0].shopcode:"";          
			for(var j=0;j<selected.length;j++){            
				codes = codes+","+selected[j].shopcode;          
			}
			$.ajaxSettings.async = false;
			$.post("/base/common/querySysShopsList",codes).done(
					function (resp) {            
						if (resp.success && resp.result.length > 0){              
							list = resp.result;              
							for(var j = 0; j<selected.length; j++){
								for(var i = 0; i<list.length; i++){
									if(list[i].shopCode == selected[j].shopcode){
										selected[j].label = list[i].shopName;                    
										break;                  
									}                
								}              
							}
							for(var ii = 0; ii<rows.length; ii++){
								for(var jj = 0; jj<selected.length; jj++){
									if(rows[ii].shopcode== selected[jj].shopcode){
										that.$refs.goodsTable.toggleRowSelection(rows[ii],true);
										break;
									}
								}
							}            
		...
		selectRow: function(selected,row){
			...
			for(var i = 0; i< selected.length; i++){
				if(selected[i].shopcode == row.shopcode){
					check = true;
					break;
				}
			}
			if(check){
				...
				for(var j = 0; j < showSelectedList.length; j++){
					if(showSelectedList[j].shopcode == row.shopcode){
						this.deleteSelectedGoods('goodsSelectedData',j,showSelectedList[j].shopcode);
						break;
					}			
				}
			}
