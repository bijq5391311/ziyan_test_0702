            ...
            var vue = new Vue({
					...
					methods: {
						...
						"initPointRules": function () {
							...
							var that = this;
							$.ajax({
								url: "/integral/zyonlinepointrule/initOnlinePointRule", // 获取到拥有品牌下的规则
								data: "",
								async: false,
								type: "post",
							...
							that.$refs.form[index].validate(function (state) {
								if (state) {
									var rule = {
										...
										is_jhs: that.pointRules[index].is_jhs,
									...
									$.ajax(
											{
												url: "/integral/zyonlinepointrule/saveOrUpdateOnline",
												type: "post",
