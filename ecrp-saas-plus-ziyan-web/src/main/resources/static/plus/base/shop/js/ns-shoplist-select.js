...
define(['jquery', 'vue', 'nui','/public/ecrp/components/zy-ns-area.js','/public/ecrp/common/common.js','/public/ecrp/subdivision/components/ns-cascader.js'], function ($, Vue, nui, areas, common) {
	...
	var props = {
		...
		goodsTableUrl: {
			type: String,
			default:function(){
				return ctx + "/marketing/zyofflinecoupon/queryOutShopTreeNew";
			}
		},
		goodsCategoryUrl:{
			type: String,
	...
	var $methods = {
     ...
     this.$set(this,'goodsSelectedData',[].concat(shopCodeList));
		...
		toggleSelection: function(selected,rows){
			var that = this;
			var list = [];
			var codes = selected.length>0?selected[0].code:"";
			for(var j=0;j<selected.length;j++){
				codes = codes+","+selected[j].code;
			}
			$.ajaxSettings.async = false;
			$.post("/base/zycommon/querySysShopsList",codes).done(function (resp) {
				if (resp.success && resp.result.length > 0){
					list = resp.result;
					for(var i = 0; i<list.length; i++){
						for(var j = 0; j<selected.length; j++){
							if(list[i].shopCode == selected[j].code ){
			...
			for(var ii = 0; ii<rows.length; ii++){
				for(var jj = 0; jj<selected.length; jj++){
					if(rows[ii].code == selected[jj].code ){
						this.$refs.goodsTable.toggleRowSelection(rows[ii],true);
						break;
					}
				}
			}
